...
export class ListComponent implements OnInit {
  ...
  add(){
    ...
    this.http.post('http://localhost:8080/store', {task: this.data, status: 'pending', priority: this.priority, date: this.date}).subscribe(
      (data)=>{
  ...
  remove(l, i){
    ...
    this.http.post('http://localhost:8080/remove', this.list[i]).subscribe(
      (data)=>{
  ...
  edit(l){
    ...
    this.http.post('http://localhost:8080/update', t).subscribe(
      (data)=>{
        l.task = temp;
